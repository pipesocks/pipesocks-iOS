...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String : NSObject]? = nil, completionHandler: @escaping (Error?) -> Void) {
        ...
        let pipesocksAdapterFactory=PipesocksAdapterFactory.init(remoteHost: config["remoteHost"] as! String, remotePort: config["remotePort"] as! UInt16, password: config["password"] as! String, enableIPv6: config["enableIPv6"] as! Bool)
        if config["autoMode"] as! Bool {
            let countryRule=CountryRule.init(countryCode: "CN", match: true, adapterFactory: DirectAdapterFactory.init())
            let allRule=AllRule.init(adapterFactory: pipesocksAdapterFactory)
            let manager=RuleManager(fromRules: [countryRule,allRule], appendDirect: true)
